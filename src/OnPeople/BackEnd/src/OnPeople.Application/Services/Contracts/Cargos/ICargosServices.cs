...
namespace OnPeople.Application.Services.Contracts.Cargos
{
    ...
    {
        Task<PageList<CargoDto>> GetAllCargosAsync(PageParameters pageParameters, int empresaId, int departamentoId, int cargoId);
        ...
        Task<DashboardCargos> GetDashboardCArgos(int empresaId, int departamentoId, int cargoId);
        Task<List<ListaMetas>> GetDashboardCargoMetas(int empresaId, int departamentoId, int cargoId);
    }
}